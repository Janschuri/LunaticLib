...
public class LunaticHelpCommand extends LunaticCommand implements HasParentCommand, HasParams {
    ...
    public Component noNumberMessage(Sender sender, String[] args) {
        return command.getHelpCommand().noNumberMessage(sender, args);
    }
    ...
    public boolean execute(Sender sender, String[] args) {
        if (args.length == 0) {
            ...
            try {
                ...
                sender.sendMessage(languageConfig.getPrefix().append(noNumberMessage(sender, args)));
            }
        }
    ...
    private Component getHelpMessage(Sender sender, int page) {
        Component header = command.getHelpHeader();
        ...
        Component footer = command.getHelpFooter(page, (int) Math.ceil((double) messages.size() /pageSize));
        builder.append(Component.newline());
