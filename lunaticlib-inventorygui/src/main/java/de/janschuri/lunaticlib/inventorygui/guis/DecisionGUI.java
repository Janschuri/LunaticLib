package de.janschuri.lunaticlib.inventorygui.guis;

import de.janschuri.lunaticlib.inventorygui.Logger;
import de.janschuri.lunaticlib.inventorygui.LunaticLibInventoryGui;
    ...
    public DecisionGUI(DecisionMessage decisionMessage) {
        ...
        this.acceptConsumer = event -> {
            ...
            performCommand(player, commandToExecute)
                    .thenAccept(success -> {
                        if (success) {
                            Bukkit.getScheduler().runTask(LunaticLibInventoryGui.getPluginInstance(), () -> player.closeInventory());
                        } else {
                            Logger.error("Error while executing command: " + commandToExecute);
                        }
                    });
        };
